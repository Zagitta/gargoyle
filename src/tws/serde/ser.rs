...
where
    W: std::io::Write,
{
    fn should_emit_seperator(&mut self) -> bool {
        let res = !self.outermost || self.len != 0usize;
    ...
    W: std::io::Write,
{
    ...
    fn serialize_i8(self, v: i8) -> Result<()> {
        itoa::write(&mut self.writer, v).map_err(|_| Error::Syntax)?;
        Ok(())
    }
    ...
    fn serialize_f32(self, v: f32) -> Result<()> {
        ...
        self.writer
            .write_all(buffer.format(v).as_bytes())
    ...
    fn serialize_char(self, _v: char) -> Result<()> {
        unimplemented!()
        /* self.writer.write_all(v).map_err(|_| Error::Syntax)?;
        Ok(()) */
    }
    ...
    fn serialize_seq(self, len: Option<usize>) -> Result<Self::SerializeSeq> {
        let old_outermost = self.outermost;
        self.outermost = false;
        Ok(Compound {
            ser: self,
            outermost: old_outermost,
            len: len.expect("We can't do this without lenghts"),
        })
    }
    ...
    fn serialize_map(self, len: Option<usize>) -> Result<Self::SerializeMap> {
        ...
        Ok(Compound {
            ser: self,
            outermost,
            len: len.expect("Can't do this without len"),
        })
    }
    ...
    fn serialize_struct_variant(
        ...
        //todo check this
        variant.serialize(&mut *self)?;
        if len > 0 || !self.outermost {
            self.writer.write_all(b"\0").map_err(|_| Error::Syntax)?;
        }
